...
# 训练集和测试集
train_dataset = datasets.ImageFolder(root='DDR/train', transform=data_transform1) 
test_dataset = datasets.ImageFolder(root='DDR/test', transform=data_transform2)  
...
class ConvMixer(nn.Module):
    def __init__(self,model, dim, depth, kernel_size=9, patch_size=7, n_classes=5):
        super().__init__()
        self.resnet_layer = nn.Sequential(*list(model.children())[:-2])
        self.transion_layer = nn.ConvTranspose2d(2048, 3, kernel_size=14, stride=3) 
...
net = ConvMixer(model,dim=384, depth=8, kernel_size=7, patch_size=8, n_classes=5)
if use_gpu:
    net = net.to(device)

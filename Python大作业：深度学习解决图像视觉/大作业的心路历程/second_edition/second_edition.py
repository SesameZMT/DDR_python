...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(         
            nn.Conv2d(                      # 卷积层
                in_channels=3,              # 输入特征图个数
                out_channels=25,            # 要得到多少个特征图，25个卷积核
        ...
        self.conv2 = nn.Sequential(         # 下一个套餐的输入 (25, 256, 256)
            nn.Conv2d(25, 50, 5, 1, 2),     # 输出 (50, 256, 256)
            nn.ReLU(),                      # relu层
            nn.MaxPool2d(2),                # 进行池化操作（2x2 区域）, 输出 (50, 128, 128)
        )
        self.conv3 = nn.Sequential(         
            nn.Conv2d(                      # 卷积层
                in_channels=50,              # 灰度图，输入特征图个数
                out_channels=100,            # 要得到多少个特征图，25个卷积核
                kernel_size=5,              # 卷积核大小，此时为5*5
                stride=1,                   # 步长
                padding=2,                  # 也就是加几圈0，如果希望卷积后大小跟原来一样，需要设置padding=(kernel_size-1)/2 if stride=1
            ),                              # 输出的特征图为 (100, 128, 128)
            nn.ReLU(),                      # relu层，激活函数
            nn.MaxPool2d(2),                # 进行池化操作（2x2 区域）,输出结果为： (100, 64, 64)
        )
        self.conv4 = nn.Sequential(         # 下一个套餐的输入 (100, 64, 64)
            nn.Conv2d(100, 200, 5, 1, 2),   # 输出 (100, 64, 64)
            nn.ReLU(),                      # relu层
            nn.MaxPool2d(2),                # 进行池化操作（2x2 区域）, 输出 (200, 32, 32)
        )
        self.out = nn.Linear(200 * 32 * 32, 5)   # 全连接层得到的结果
    ...
    def forward(self, x):
        ...
        x = self.conv3(x)
        x = self.conv4(x)
        x = x.view(x.size(0), -1)           # flatten操作，结果为：(batch_size, 200 * 32 * 32)
        output = self.out(x)
        return output
